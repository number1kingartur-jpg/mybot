...
const PHOTO_MISMATCH_HOLD = new Set([
  ...
  "w8_coast_road",
  "w8_market_morning",
  // Аудит 08.09.2026: все шесть уже опубликованы (входили в те же 37 постов,
  // что и остальные), значит из канала это не убирает. Смысл записи один,
  // это защита на случай потери состояния "уже опубликовано" (это уже
  // случалось при redeploy без Volume, см. REPORTS/BABKI.md) - тогда бот
  // не отправит их повторно с тем же плохим фото.
  "w5_case_start", // фото другого человека, не Артур с эталонных фото
  "w5_shoulder", // фото другого человека (татуировка-крыло, стадион)
  "w5_scale_lies", // photo-map ссылается на фото ног на весах, а реально
  // отправлялось фото третьего незнакомого человека - карта и факт разошлись
  "w6_belt", // обычный кожаный ремень вместо атлетического пояса
  "w8_outdoor_gym", // крупный план рук на штанге, ни моря, ни турника
  "w8_bay_evening", // тот же кадр, что уже стоит в w6_hr, не по теме поста
]);
