...
check("яичница в основных", matchFood("яичница")?.role === "protein");
check("омлет в основных", matchFood("омлет")?.role === "protein");
check("блины в основных", matchFood("блины")?.role === "carb");
check("оладьи в основных", matchFood("оладьи")?.role === "carb");
check("сырники в основных", matchFood("сырники")?.role === "protein");
const threeBliny = macrosFromText("3 блина");
near("3 блина", threeBliny?.parts?.[0]?.grams ?? 0, 150, 0.01);
check("3 блина в названии", /3 блина/i.test(threeBliny?.name ?? ""), threeBliny?.name);
const fourOladi = macrosFromText("4 оладьи");
near("4 оладьи", fourOladi?.parts?.[0]?.grams ?? 0, 140, 0.01);
check("4 оладьи в названии", /4 оладьи/i.test(fourOladi?.name ?? ""), fourOladi?.name);
const boiledFour = macrosFromText("4 яйца отварные");
near("4 яйца отварные", boiledFour?.parts?.[0]?.grams ?? 0, 220, 0.01);
check("4 яйца отварные в названии", /4 яйца отварные/i.test(boiledFour?.name ?? ""), boiledFour?.name);
